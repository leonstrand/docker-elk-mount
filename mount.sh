...
for host in $hosts; do
  ...
  if [ -n "$ip" ]; then
    ...
    for directory in $directories; do
      ...
      if ! grep -q '^\s*//'$ip'/'$prefix'/'$directory'\s*/pai-logs/'$host'/'$directory'/\s*cifs\s*username='$cifs_username',password='$cifs_password',_netdev,ro\s*0\s*0$' /etc/fstab; then
        ...
        echo echo \'//$ip/$prefix/$directory /pai-logs/$host/$directory/ cifs username=$cifs_username,password=$cifs_password,_netdev,ro 0 0\' \| sudo tee -a /etc/fstab #verbose
        echo '//'$ip'/'$prefix'/'$directory' /pai-logs/'$host'/'$directory'/ cifs username='$cifs_username',password='$cifs_password',_netdev,ro 0 0' | sudo tee -a /etc/fstab
